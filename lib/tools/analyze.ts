...
const VISION_MODEL =
  process.env.VOLC_VISION_MODEL || "doubao-seed-2-0-pro-260215";
const TEXT_ANALYZE_MODEL =
  process.env.VOLC_ANALYZE_TEXT_MODEL ||
  process.env.VOLC_ANSWER_MODEL ||
  VISION_MODEL;
const ANALYZE_TIMEOUT_MS = Number(process.env.ANALYZE_TIMEOUT_MS || 8000);
...
/**
 * 创建 analyzeTool。接受 image_b64 闭包参数，避免全局变量。
 */
export function createAnalyzeTool(image_b64: string) {
  return tool({
    description:
    ...
    execute: async ({ text }) => {
      ...
      const model = image_b64 ? VISION_MODEL : TEXT_ANALYZE_MODEL;

      const request = volcClient.chat.completions.create({
        model,
      ...
      let resp: Awaited<typeof request>;
      try {
        ...
        return {
          is_math: true,
          questions: [{ text, knowledge_points: fallbackKps }],
        };
      }
      ...
      let analysis: {
        is_math: boolean;
        questions: { text: string; knowledge_points: string[] }[];
      };
      try {
        ...
        analysis = {
          is_math: true,
          questions: [{ text, knowledge_points: [] }],
        };
      }
